...
const glassInputs=Array.from(document.querySelectorAll("[data-glass-setting]"));
const glassSurfaceElements=Array.from(document.querySelectorAll(".sphere-topbar,.sphere-footer-nav"));
...
const STORAGE_GLASS="portfolioSphere.glassSettings.v2";
...
const glassDefaults={refraction:90,depth:45,dispersion:20,frost:6,splay:72,opacity:88};
let glassState={...glassDefaults};
let glassRefractionSurfaces=[];
let glassRefractionLastAt=0;
...
function loadGlassSettings(){try{return{...glassDefaults,...JSON.parse(localStorage.getItem(STORAGE_GLASS)||"{}")};}catch(error){return{...glassDefaults};}}
function updateGlassUi(save=true){glassInputs.forEach((input)=>{glassState[input.dataset.glassSetting]=Math.round(clampSetting(input.value,glassDefaults[input.dataset.glassSetting],0,100));document.querySelector(`[data-glass-value="${input.dataset.glassSetting}"]`).textContent=String(glassState[input.dataset.glassSetting]);});spherePage.style.setProperty("--glass-opacity",(0.025+(1-glassState.opacity/100)*0.16).toFixed(3));spherePage.style.setProperty("--glass-blur",`${(glassState.frost*0.28).toFixed(1)}px`);spherePage.style.setProperty("--glass-frost-scale",(1+glassState.frost*0.0012).toFixed(3));spherePage.style.setProperty("--glass-border-alpha",(0.08+glassState.splay*0.0012).toFixed(3));spherePage.style.setProperty("--glass-refraction-mix",(0.35+glassState.opacity/100*0.62).toFixed(2));spherePage.style.setProperty("--glass-depth-y",`${Math.round(glassState.depth*0.06)}px`);spherePage.style.setProperty("--glass-depth-blur",`${Math.round(12+glassState.depth*0.2)}px`);spherePage.style.setProperty("--glass-depth-alpha",(glassState.depth*0.0009).toFixed(3));spherePage.style.setProperty("--glass-dispersion-offset",`${(glassState.dispersion*0.07).toFixed(1)}px`);spherePage.style.setProperty("--glass-dispersion-alpha",(glassState.dispersion*0.0024).toFixed(3));spherePage.style.setProperty("--glass-splay",(glassState.splay/100).toFixed(2));if(save)localStorage.setItem(STORAGE_GLASS,JSON.stringify(glassState));}
function initializeGlassControls(){glassState=loadGlassSettings();glassRefractionSurfaces=glassSurfaceElements.map((element)=>{const surface=document.createElement("canvas");const buffer=document.createElement("canvas");surface.className="glass-refraction-canvas";surface.setAttribute("aria-hidden","true");element.prepend(surface);return{element,surface,context:surface.getContext("2d"),buffer,bufferContext:buffer.getContext("2d")};});glassInputs.forEach((input)=>{input.value=String(glassState[input.dataset.glassSetting]??glassDefaults[input.dataset.glassSetting]);input.addEventListener("input",()=>updateGlassUi(true));});updateGlassUi(false);}
function updateGlassRefraction(now){if(viewMode!=="sphere"||now-glassRefractionLastAt<40)return;glassRefractionLastAt=now;const strength=glassState.refraction/100;const dispersion=glassState.dispersion/100;glassRefractionSurfaces.forEach(({element,surface,context,buffer,bufferContext})=>{const rect=element.getBoundingClientRect();const surfaceWidth=Math.max(1,Math.round(rect.width));const surfaceHeight=Math.max(1,Math.round(rect.height));if(surface.width!==surfaceWidth||surface.height!==surfaceHeight){surface.width=surfaceWidth;surface.height=surfaceHeight;buffer.width=surfaceWidth;buffer.height=surfaceHeight;}context.clearRect(0,0,surfaceWidth,surfaceHeight);bufferContext.clearRect(0,0,surfaceWidth,surfaceHeight);if(rect.width<=1||rect.height<=1)return;const strips=24;const curve=strength*0.46;const map=(value)=>value+curve*value*value*value*(1-value*value);for(let strip=0;strip<strips;strip++){const dx0=surfaceWidth*strip/strips;const dx1=surfaceWidth*(strip+1)/strips;const unit0=dx0/surfaceWidth*2-1;const unit1=dx1/surfaceWidth*2-1;const sx0=rect.left+(map(unit0)+1)*rect.width*0.5;const sx1=rect.left+(map(unit1)+1)*rect.width*0.5;const sourceX=Math.max(0,Math.min(width,sx0));const sourceRight=Math.max(sourceX+0.5,Math.min(width,sx1));const sourceY=Math.max(0,Math.min(height,rect.top));const sourceBottom=Math.max(sourceY+0.5,Math.min(height,rect.bottom));bufferContext.drawImage(canvas,sourceX*dpr,sourceY*dpr,(sourceRight-sourceX)*dpr,(sourceBottom-sourceY)*dpr,dx0,0,dx1-dx0,surfaceHeight);}context.drawImage(buffer,0,0);if(dispersion>0.001){const offset=dispersion*7;const alpha=dispersion*0.24;context.save();context.globalCompositeOperation="screen";context.globalAlpha=alpha;context.filter="sepia(1) saturate(8) hue-rotate(305deg)";context.drawImage(buffer,-offset,0);context.filter="sepia(1) saturate(8) hue-rotate(145deg)";context.drawImage(buffer,offset,0);context.restore();}});}
...
window.addEventListener("resize",resize);resize();updateUi();updateProjectUi();initializeGlassControls();loadItems(assets);renderCvCylinderTabs();Promise.all([loadStoredAssetsAsync(),loadStoredCvNodesAsync()]).then(([nextAssets,nextCvNodes])=>{assets=nextAssets;cvNodes=nextCvNodes;loadItems(assets);renderCvCylinderTabs();setCvLook(cvLook);});render();})();
