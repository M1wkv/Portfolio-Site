...
const STORAGE_GLASS="portfolioSphere.glassSettings";
...
const glassDefaults={refraction:40,depth:100,dispersion:100,frost:5,splay:94,opacity:75};
...
function updateGlassUi(save=true){glassInputs.forEach((input)=>{glassState[input.dataset.glassSetting]=Math.round(clampSetting(input.value,glassDefaults[input.dataset.glassSetting],0,100));document.querySelector(`[data-glass-value="${input.dataset.glassSetting}"]`).textContent=String(glassState[input.dataset.glassSetting]);});spherePage.style.setProperty("--glass-opacity",(glassState.opacity/100).toFixed(2));spherePage.style.setProperty("--glass-blur",`${(glassState.frost*0.28).toFixed(1)}px`);spherePage.style.setProperty("--glass-frost-scale",(1+glassState.frost*0.0012).toFixed(3));spherePage.style.setProperty("--glass-border-alpha",(0.06+glassState.depth*0.0006).toFixed(3));spherePage.style.setProperty("--glass-refraction-mix",(0.18+(1-glassState.opacity/100)*0.82).toFixed(2));spherePage.style.setProperty("--glass-depth-y",`${Math.round(glassState.depth*0.12)}px`);spherePage.style.setProperty("--glass-depth-blur",`${Math.round(12+glassState.depth*0.32)}px`);spherePage.style.setProperty("--glass-depth-alpha",(glassState.depth*0.0024).toFixed(3));spherePage.style.setProperty("--glass-dispersion-offset",`${(glassState.dispersion*0.035).toFixed(1)}px`);spherePage.style.setProperty("--glass-dispersion-alpha",(glassState.dispersion*0.0012).toFixed(3));spherePage.style.setProperty("--glass-splay",(glassState.splay/100).toFixed(2));if(save)localStorage.setItem(STORAGE_GLASS,JSON.stringify(glassState));}
