...
function loadIdleSettings(){try{return{...idleDefaults,...JSON.parse(localStorage.getItem("portfolioSphere.idlePanelEffects.v5")||"{}")};}catch{return{...idleDefaults};}}
function updateIdleUi(save=true){idleInputs.forEach((input)=>{const key=input.dataset.idleSetting;idleState[key]=clampSetting(input.value,idleDefaults[key],0,100);const value=document.querySelector(`[data-idle-value="${key}"]`);if(value)value.textContent=String(Math.round(idleState[key]));});const baseAlpha=1-idleState.transparency/100;const frostCarrier=idleState.frost>0?idleState.frost*0.0008:0;const alpha=Math.max(baseAlpha,frostCarrier);const rgb=Math.round(28*(1-idleState.darkening/100));["top","bottom"].forEach((panel)=>{spherePage.style.setProperty(`--idle-${panel}-alpha`,alpha.toFixed(3));spherePage.style.setProperty(`--idle-${panel}-rgb`,String(rgb));spherePage.style.setProperty(`--idle-${panel}-blur`,`${(idleState.frost*0.18).toFixed(1)}px`);});if(save){try{localStorage.setItem("portfolioSphere.idlePanelEffects.v5",JSON.stringify(idleState));}catch{}}}
