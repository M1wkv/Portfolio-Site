...
function setCvFace(face,immediate=false){if(!cvFaceTargets[face])return;cvActiveFace=face;const target=cvFaceTargets[face];cvTargetBox={yaw:cvNearestAngle(cvBoxYaw,target.yaw),pitch:target.pitch};if(immediate){cvBoxYaw=cvTargetBox.yaw;cvBoxPitch=cvTargetBox.pitch;}cvBoxVelocity={yaw:0,pitch:0};cvSectionButtons.forEach((button)=>button.classList.toggle("is-active",button.dataset.cvFace===face));}
function updateCvSceneUi(){if(!cvSphereSizeRange)return;cvSceneState.sphereSize=Number(cvSphereSizeRange.value);cvSceneState.blur=Number(cvOverlayBlurRange.value);cvSceneState.dim=Number(cvOverlayDimRange.value);cvSphereSizeValue.textContent=cvSceneState.sphereSize.toFixed(2);cvOverlayBlurValue.textContent=String(Math.round(cvSceneState.blur));cvOverlayDimValue.textContent=String(Math.round(cvSceneState.dim));}
