...
const waterDefaults={transparency:50,darkening:50,frost:5};
let waterState={...waterDefaults};
const idleDefaults={transparency:100,darkening:100,frost:20};
let idleState={top:{...idleDefaults},bottom:{...idleDefaults}};
...
function loadIdleSettings(){try{const stored=JSON.parse(localStorage.getItem("portfolioSphere.idlePanelEffects.v4")||"{}");return{top:{...idleDefaults,...stored.top},bottom:{...idleDefaults,...stored.bottom}};}catch{return{top:{...idleDefaults},bottom:{...idleDefaults}};}}
function updateIdleUi(save=true){idleInputs.forEach((input)=>{const panel=input.dataset.idlePanel||"top";const key=input.dataset.idleSetting;idleState[panel][key]=clampSetting(input.value,idleDefaults[key],0,100);const value=document.querySelector(`[data-idle-value="${panel}-${key}"]`);if(value)value.textContent=String(Math.round(idleState[panel][key]));});["top","bottom"].forEach((panel)=>{const values=idleState[panel];const baseAlpha=1-values.transparency/100;const frostCarrier=values.frost>0?values.frost*0.0008:0;const alpha=Math.max(baseAlpha,frostCarrier);const rgb=Math.round(28*(1-values.darkening/100));spherePage.style.setProperty(`--idle-${panel}-alpha`,alpha.toFixed(3));spherePage.style.setProperty(`--idle-${panel}-rgb`,String(rgb));spherePage.style.setProperty(`--idle-${panel}-blur`,`${(values.frost*0.18).toFixed(1)}px`);});if(save){try{localStorage.setItem("portfolioSphere.idlePanelEffects.v4",JSON.stringify(idleState));}catch{}}}
function setPanelIdle(panel,idle){if(viewMode!=="sphere")return;spherePage.classList.toggle(`is-ui-idle-${panel}`,idle);}
function schedulePanelIdle(panel){window.clearTimeout(idleTimers[panel]);idleTimers[panel]=window.setTimeout(()=>setPanelIdle(panel,true),120);}
function initializeIdleEffects(){idleState=loadIdleSettings();idleInputs.forEach((input)=>{const panel=input.dataset.idlePanel||"top";const key=input.dataset.idleSetting;input.value=String(clampSetting(idleState[panel][key],idleDefaults[key],0,100));input.addEventListener("input",()=>updateIdleUi(true));});updateIdleUi(false);const surfaces={top:topbar,bottom:footerNav};Object.entries(surfaces).forEach(([panel,surface])=>{if(!surface)return;const wake=()=>{window.clearTimeout(idleTimers[panel]);setPanelIdle(panel,false);};surface.addEventListener("pointerenter",wake);surface.addEventListener("pointerleave",()=>schedulePanelIdle(panel));});spherePage.addEventListener("pointermove",(event)=>{Object.entries(surfaces).forEach(([panel,surface])=>{if(!surface)return;const rect=surface.getBoundingClientRect();const isOver=event.clientX>=rect.left&&event.clientX<=rect.right&&event.clientY>=rect.top&&event.clientY<=rect.bottom;if(isOver){window.clearTimeout(idleTimers[panel]);setPanelIdle(panel,false);}else schedulePanelIdle(panel);});},true);schedulePanelIdle("top");schedulePanelIdle("bottom");}
...
window.addEventListener("resize",resize);resize();updateUi();updateProjectUi();initializeWaterEffects();initializeIdleEffects();loadItems(assets);Promise.all([loadStoredAssetsAsync(),loadStoredCvNodesAsync()]).then(([nextAssets,nextCvNodes])=>{assets=nextAssets;cvNodes=nextCvNodes;loadItems(assets);});render();})();
