...
const cvProjectNav=document.querySelector(".cv-project-nav");
...
const projectRoleTags=document.getElementById("projectRoleTags");
...
const viewportProfile=window.matchMedia("(max-width: 767px)").matches?"mobile":window.matchMedia("(max-width: 1023px)").matches?"tablet":"desktop";const responsiveSceneSettings={desktop:{sphereSize:clampSetting(sphereSettings.size,0.6,0.1,1),elementScale:clampSetting(sphereSettings.elementScale,0.6,0.1,1),itemCount:clampSetting(sphereSettings.itemCount,50,10,100),fisheye:clampSetting(sphereSettings.fisheye,0.15,0,1),rotationX:clampSetting(sphereSettings.rotationX,0.14,-0.32,0.32),rotationY:clampSetting(sphereSettings.rotationY,-0.09,-0.32,0.32),projectScale:clampSetting(sphereSettings.projectScale,0.5,0.1,0.9),projectItemCount:clampSetting(sphereSettings.projectItemCount,20,1,100),projectGap:clampSetting(sphereSettings.projectGap,0.5,0.5,2),projectWidth:clampSetting(sphereSettings.projectWidth,0.75,0.6,1.6),projectLength:clampSetting(sphereSettings.projectLength,1.25,0.5,1.8)},tablet:{sphereSize:clampSetting(sphereSettings.tabletSize,0.65,0.1,1),elementScale:clampSetting(sphereSettings.tabletElementScale,0.42,0.1,1),itemCount:clampSetting(sphereSettings.tabletItemCount,50,10,100),fisheye:clampSetting(sphereSettings.tabletFisheye,0.15,0,1),rotationX:clampSetting(sphereSettings.tabletRotationX,0.14,-0.32,0.32),rotationY:clampSetting(sphereSettings.tabletRotationY,-0.09,-0.32,0.32),projectScale:clampSetting(sphereSettings.tabletProjectScale,0.44,0.1,0.9),projectItemCount:clampSetting(sphereSettings.tabletProjectItemCount,16,1,100),projectGap:clampSetting(sphereSettings.tabletProjectGap,0.5,0.5,2),projectWidth:clampSetting(sphereSettings.tabletProjectWidth,0.7,0.6,1.6),projectLength:clampSetting(sphereSettings.tabletProjectLength,1.08,0.5,1.8)},mobile:{sphereSize:clampSetting(sphereSettings.mobileSize,0.7,0.1,1),elementScale:clampSetting(sphereSettings.mobileElementScale,0.24,0.1,1),itemCount:clampSetting(sphereSettings.mobileItemCount,40,10,100),fisheye:clampSetting(sphereSettings.mobileFisheye,0.15,0,1),rotationX:clampSetting(sphereSettings.mobileRotationX,0.14,-0.32,0.32),rotationY:clampSetting(sphereSettings.mobileRotationY,-0.09,-0.32,0.32),projectScale:clampSetting(sphereSettings.mobileProjectScale,0.38,0.1,0.9),projectItemCount:clampSetting(sphereSettings.mobileProjectItemCount,12,1,100),projectGap:clampSetting(sphereSettings.mobileProjectGap,0.5,0.5,2),projectWidth:clampSetting(sphereSettings.mobileProjectWidth,0.66,0.6,1.6),projectLength:clampSetting(sphereSettings.mobileProjectLength,0.9,0.5,1.8)}};const activeSceneSettings=responsiveSceneSettings[viewportProfile];const MAX_VISIBLE_ITEMS=Math.round(activeSceneSettings.itemCount);sizeRange.value=String(activeSceneSettings.sphereSize);scaleRange.value=String(activeSceneSettings.elementScale);fisheyeRange.value=String(activeSceneSettings.fisheye);projectScaleRange.value=String(activeSceneSettings.projectScale);projectCountRange.value=String(Math.round(activeSceneSettings.projectItemCount));projectGapRange.value=String(activeSceneSettings.projectGap);projectWidthRange.value=String(activeSceneSettings.projectWidth);projectLengthRange.value=String(activeSceneSettings.projectLength);document.documentElement.dataset.sphereMaxVisibleItems=String(MAX_VISIBLE_ITEMS);document.documentElement.dataset.viewportProfile=viewportProfile;
...
const bootstrapProjects=(Array.isArray(bootstrapData.projects)?bootstrapData.projects:window.PORTFOLIO_BOOTSTRAP?.projects||[]).filter((project)=>project&&project.projectId).map((project)=>({projectId:String(project.projectId),title:String(project.title||"Проект"),description:String(project.description||""),tools:String(project.tools||""),timeline:String(project.timeline||""),task:String(project.task||""),idea:String(project.idea||""),role:String(project.role||"")}));
...
let visibleVideoItems=new Set();
let expandedVideoItem=null;
let width=0;
let height=0;
let dpr=1;
...
let direction={x:activeSceneSettings.rotationX,y:activeSceneSettings.rotationY};
let hovering=false;
...
const cvSceneState={sphereSize:0.72,elementScale:0.6,dim:45};
...
function resize(){const mobileDpr=window.innerWidth<768?1.35:2;dpr=Math.min(window.devicePixelRatio||1,mobileDpr);width=window.innerWidth;height=window.innerHeight;canvas.width=Math.round(width*dpr);canvas.height=Math.round(height*dpr);ctx.setTransform(dpr,0,0,dpr,0,0);updateProjectMediaSafeMargin();}
...
function isVideoAsset(asset){const source=String(asset?.src||asset||"").split("?")[0].toLowerCase();const type=String(asset?.mediaType||asset?.type||"").toLowerCase();return type==="video"||type.startsWith("video/")||source.startsWith("data:video/")||/\.(mp4|webm)$/.test(source);}
function mediaWidth(item){return item?.mediaType==="video"?item.img.videoWidth:item?.img?.naturalWidth;}
function mediaHeight(item){return item?.mediaType==="video"?item.img.videoHeight:item?.img?.naturalHeight;}
function startMediaItemLoad(item){if(item.loadStarted)return;item.loadStarted=true;const markLoaded=()=>{item.loaded=true;item.loadFailed=false;if(item.trackDiagnostics)exposeLoadedProjectDiagnostics();};item.img.onerror=()=>{item.loaded=false;item.loadFailed=true;if(item.trackDiagnostics)exposeLoadedProjectDiagnostics();};if(item.mediaType==="video"){item.img.onloadeddata=markLoaded;item.img.oncanplay=markLoaded;item.img.src=item.src;item.img.load();return;}item.img.onload=markLoaded;item.img.src=item.src;}
function scheduleMediaItemLoads(nextItems){const initialCount=window.innerWidth<768?8:12;nextItems.slice(0,initialCount).forEach(startMediaItemLoad);let cursor=initialCount;const loadBatch=()=>{nextItems.slice(cursor,cursor+4).forEach(startMediaItemLoad);cursor+=4;if(cursor<nextItems.length){if("requestIdleCallback"in window)requestIdleCallback(loadBatch,{timeout:1200});else setTimeout(loadBatch,180);}};if(cursor<nextItems.length){if("requestIdleCallback"in window)requestIdleCallback(loadBatch,{timeout:900});else setTimeout(loadBatch,220);}}
function createMediaItems(nextAssets,trackDiagnostics,eager=true){const nextItems=nextAssets.map((asset,index)=>{const video=isVideoAsset(asset);const img=video?document.createElement("video"):new Image();if(video){img.muted=true;img.defaultMuted=true;img.loop=true;img.playsInline=true;img.preload="metadata";img.crossOrigin="anonymous";}else{img.decoding="async";}return{img,mediaType:video?"video":"image",src:asset.src,title:asset.title||`Work ${index + 1}`,projectId:asset.projectId||asset.project_id||"",loaded:false,loadFailed:false,loadStarted:false,playPending:false,trackDiagnostics};});if(eager)scheduleMediaItemLoads(nextItems);return nextItems;}
...
function imageRatio(item){const mediaH=mediaHeight(item);return item.loaded&&mediaH?mediaWidth(item)/mediaH:0.74;}
...
const hitH=cardH*Math.max(radialScale,tangentScale);if(alpha>0.02)entry.hit={item,index:entry.index,x,y,z,w:hitW,h:hitH};else delete entry.hit;if(item.mediaType==="video"&&viewMode!=="cv"&&alpha>0.03&&x+hitW/2>0&&x-hitW/2<width&&y+hitH/2>0&&y-hitH/2<height)visibleVideoItems.add(item);ctx.save();ctx.translate(x,y);if(mode==="ribbon"){const side=Math.max(-1,Math.min(1,(x-width/2)/Math.max(1,width*0.5)));
const faceTurn=entry.faceTurn||0;
const compress=Math.max(0.42,1-Math.abs(faceTurn)*0.38);ctx.transform(compress,0,0,1,0,0);}else{ctx.rotate(angle);ctx.scale(radialScale,tangentScale);ctx.rotate(-angle);}ctx.globalAlpha=alpha;if(item.loaded&&mediaWidth(item)){ctx.drawImage(item.img,-cardW/2,-cardH/2,cardW,cardH);}else{const gradient=ctx.createLinearGradient(-cardW/2,-cardH/2,cardW/2,cardH/2);gradient.addColorStop(0,"rgba(255,255,255,0.18)");gradient.addColorStop(1,"rgba(255,255,255,0.04)");ctx.fillStyle=gradient;ctx.fillRect(-cardW/2,-cardH/2,cardW,cardH);}ctx.restore();}
function allVideoItems(){return[...new Set(items.concat(projectMediaItems).filter((item)=>item.mediaType==="video"))];}
function requestVideoPlayback(item){if(!item?.loaded||!item.img.paused||item.playPending)return;item.playPending=true;const promise=item.img.play();if(promise&&typeof promise.finally==="function")promise.catch(()=>{}).finally(()=>{item.playPending=false;});else item.playPending=false;}
function syncVisibleVideoPlayback(){allVideoItems().forEach((item)=>{const expanded=item===expandedVideoItem;const shouldPlay=document.visibilityState==="visible"&&(expanded||visibleVideoItems.has(item));const shouldMute=!expanded;if(item.img.muted!==shouldMute)item.img.muted=shouldMute;item.img.defaultMuted=shouldMute;if(shouldPlay)requestVideoPlayback(item);else if(!item.img.paused)item.img.pause();});visibleVideoItems.clear();}
function closeExpandedVideo(){if(!expandedVideoItem)return;expandedVideoItem.img.muted=true;expandedVideoItem.img.defaultMuted=true;expandedVideoItem=null;}
function openExpandedVideo(item){closeExpandedVideo();if(item?.mediaType!=="video")return;expandedVideoItem=item;item.img.muted=false;item.img.defaultMuted=false;item.img.volume=1;try{item.img.currentTime=0;}catch(error){}item.playPending=false;const promise=item.img.play();if(promise&&typeof promise.catch==="function")promise.catch(()=>{item.img.muted=true;item.img.defaultMuted=true;});}
...
function focusSphereItem(item,index){if(viewMode!=="sphere"||!item)return;
const visibleItems=getVisibleItems();if(!visibleItems.length)return;
const resolvedIndex=Math.max(0,Math.min(visibleItems.length-1,Number.isFinite(index)?index:visibleItems.findIndex((entry)=>entry.src===item.src)));
const point=fibonacciPoint(resolvedIndex,visibleItems.length);
const horizontal=Math.hypot(point.x,point.z);
const targetY=Math.atan2(point.x,point.z);
const targetX=Math.atan2(point.y,horizontal);
const key=projectKey(item);sphereProjectFocusKey=key;sphereProjectFocusSrc=item.src;sphereProjectFocusTarget=1;sphereProjectFocusRotation={x:nearestAngle(rotation.x,targetX),y:nearestAngle(rotation.y,targetY)};velocity.x=0;velocity.y=0;targetVelocity.x=0;targetVelocity.y=0;document.documentElement.dataset.sphereFocusedProject=key;document.documentElement.dataset.sphereFocusedSource=item.src;setProjectIndexActive(key);}
...
const maxAngle=1.28;
const centerY=height*0.43+45-(width<768?38:0);
...
function projectLabel(item,index){const rawTitle=(item.title||"").trim();if(rawTitle&&rawTitle.length<=28&&!rawTitle.includes("_")&&!/^work\s+\d+$/i.test(rawTitle)){return rawTitle.toUpperCase();}return `DESIGN CASE ${String(index + 1).padStart(2, "0")}`;}
function projectTokenList(value){return String(value||"").split(/[\n,;|]+/).map((entry)=>entry.trim()).filter(Boolean);}
function replaceProjectTokens(target,value){if(!target)return;const labels=projectTokenList(value);const current=Array.from(target.children,(node)=>node.textContent||"");if(labels.length===current.length&&labels.every((label,index)=>label===current[index]))return;target.replaceChildren(...labels.map((label)=>{const token=document.createElement("span");token.textContent=label;return token;}));}
function projectTimelineParts(value){const raw=String(value||"").trim();const yearMatch=raw.match(/\b(?:19|20)\d{2}\b/);const year=yearMatch?.[0]||"";const period=year?raw.replace(year,"").replace(/^[\s,;|/—-]+|[\s,;|/—-]+$/g,"").trim():raw;return{period,year};}
function applyProjectContent(item,index){const project=bootstrapProjects.find((entry)=>entry.projectId===projectKey(item));const details=project||{title:item?.title||"",description:"",tools:"",timeline:"",task:"",idea:"",role:""};projectTitle.textContent=projectLabel({title:details.title||item?.title},index);if(projectSubtitle)projectSubtitle.textContent=details.description;replaceProjectTokens(projectTools,details.tools);const timeline=projectTimelineParts(details.timeline);if(projectTimeline){projectTimeline.textContent=timeline.period;projectTimeline.hidden=!timeline.period;}if(projectYear){projectYear.textContent=timeline.year;projectYear.hidden=!timeline.year;}if(projectTask)projectTask.textContent=details.task;if(projectIdea)projectIdea.textContent=details.idea;replaceProjectTokens(projectRoleTags,details.role);}
function cvTagList(value){return String(value||"").split(/[\n,;]+/).map((item)=>item.trim()).filter(Boolean);}function applyCvDisplay(data){if(!data||typeof data!=="object")return;const name=document.getElementById("cvName");const setText=(id,value)=>{if(typeof value==="string"&&value.trim()){const node=document.getElementById(id);if(node)node.textContent=value.trim();}};if(name&&typeof data.name==="string"&&data.name.trim()){const words=data.name.trim().toUpperCase().split(/\s+/);if(words.length>1){name.replaceChildren();words.forEach((word,index)=>{if(index)name.append(document.createElement("br"));name.append(document.createTextNode(word));});}}setText("cvIntro",data.intro);[["cvSkillTags",data.skills],["cvToolTags",data.tools]].forEach(([id,value])=>{const target=document.getElementById(id);const tags=cvTagList(value);if(!target||!tags.length)return;target.replaceChildren(...tags.map((tag)=>{const chip=document.createElement("span");chip.textContent=tag;return chip;}));});const list=document.getElementById("cvExperienceList");const rows=String(data.experienceItems||"").split("\n").map((line)=>line.split("|").map((part)=>part.trim())).filter((parts)=>parts[0]);if(list&&rows.length){list.replaceChildren(...rows.map(([title,company,period])=>{const article=document.createElement("article"),role=document.createElement("b"),place=document.createElement("span"),time=document.createElement("time"),roleWords=String(title||"").trim().split(/\s+/).filter(Boolean);if(roleWords.length>1){role.append(document.createTextNode(roleWords.slice(0,-1).join(" ")),document.createElement("br"),document.createTextNode(roleWords[roleWords.length-1]));}else{role.textContent=roleWords[0]||"";}place.textContent=company||"";time.textContent=(period||"").replace(/\s+-\s+/g," — ").replace(/настоящее\s+время/gi,"наст. время").replace(/(\d{4})\s*г\./gi,"$1");article.append(role,place,time);return article;}));}}const defaultCvNodes=[{look:"center",yaw:0,pitch:0,eyebrow:"CV / ART DIRECTION / AI DESIGN",title:"Alexander",body:"Creative designer building visual systems, AI-assisted image stories and interactive portfolio experiences.",type:"hero"},{look:"top",yaw:0,pitch:34,title:"Profile",body:"Visual direction, generative content, interface composition and cinematic digital cases."},{look:"bottom",yaw:0,pitch:-34,title:"Experience",body:"Portfolio systems, AI campaigns, social content packs, landing visuals and case studies."},{look:"left",yaw:-42,pitch:0,title:"Tools",body:"Figma, Photoshop, Illustrator, After Effects, Midjourney, Runway, Krea and web layout basics."},{look:"right",yaw:42,pitch:0,title:"Contact",body:"Available for visual identity, AI art direction, portfolio sites and design case packaging."}];
function cvHttpUrl(value){const raw=String(value||"").trim();if(!raw)return"";const candidate=/^https?:\/\//i.test(raw)?raw:`https://${raw.replace(/^\/+/,"")}`;try{const url=new URL(candidate);return url.protocol==="https:"||url.protocol==="http:"?url.href:"";}catch(error){return"";}}
function cvProfileUrl(value,service){const raw=String(value||"").trim();if(!raw)return"";if(/^https?:\/\//i.test(raw)||raw.includes("."))return cvHttpUrl(raw);const username=raw.replace(/^@/,"").replace(/^\/+|\/+$/g,"");if(!username)return"";if(service==="telegram")return`https://t.me/${encodeURIComponent(username)}`;if(service==="linkedin")return`https://www.linkedin.com/in/${encodeURIComponent(username)}`;if(service==="behance")return`https://www.behance.net/${encodeURIComponent(username)}`;return"";}
function applyCvContacts(data){const list=document.getElementById("cvContactLinks");if(!list)return;const section=list.closest(".cv-contacts"),contacts=data&&typeof data.contacts==="object"?data.contacts:{};const email=String(contacts.email||"").trim(),phone=String(contacts.phone||"").trim(),phoneValue=phone.replace(/[^\d+]/g,"");const items=[["LinkedIn",cvProfileUrl(contacts.linkedin,"linkedin"),true],["Telegram",cvProfileUrl(contacts.telegram,"telegram"),true],["Email",email&&/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)?`mailto:${email}`:"",false],["Телефон",phoneValue?`tel:${phoneValue}`:"",false],["Behance",cvProfileUrl(contacts.behance,"behance"),true],["Оставить заявку",cvHttpUrl(contacts.formEndpoint),true],["Скачать CV",cvHttpUrl(data?.pdfUrl),true]].filter((item)=>item[1]);list.replaceChildren(...items.map(([label,href,external])=>{const link=document.createElement("a");link.textContent=label;link.href=href;if(external){link.target="_blank";link.rel="noopener noreferrer";}return link;}));if(section)section.hidden=!items.length;}
...
const cvText=Array.isArray(cv)?cv.filter((section)=>!["pdf","contact_form"].includes(section.position)).map((section)=>[section.title,section.description].filter(Boolean).join(": ")).filter(Boolean).join(" "):"";
...
function updateCvSceneUi(){if(!cvSphereSizeRange)return;cvSceneState.sphereSize=Number(cvSphereSizeRange.value);cvSceneState.elementScale=Number(cvElementScaleRange.value);cvSceneState.dim=Number(cvOverlayDimRange.value);cvSphereSizeValue.textContent=cvSceneState.sphereSize.toFixed(2);cvElementScaleValue.textContent=cvSceneState.elementScale.toFixed(2);cvOverlayDimValue.textContent=String(Math.round(cvSceneState.dim));}
...
ctx.save();ctx.translate(center.x,center.y);ctx.globalAlpha=(active?0.96:0.52)*facing;ctx.fillStyle="#fff";ctx.textAlign="center";ctx.textBaseline="middle";ctx.font=`700 ${Math.max(9,(mobile?9:11)*textScale)}px Helvetica Neue Cyr, Helvetica Neue, Helvetica, Arial, sans-serif`;ctx.fillText(cvBoxLabel(node),0,-58*textScale);ctx.font=`700 ${Math.max(20,(node.type==="hero"?(mobile?40:58):(mobile?28:40))*textScale)}px Helvetica Neue Cyr, Helvetica Neue, Helvetica, Arial, sans-serif`;ctx.fillText(node.title,0,-12*textScale);ctx.globalAlpha=(active?0.74:0.38)*facing;const bodySize=Math.max(11,(mobile?13:15)*textScale);const bodyFont=`${bodySize}px Helvetica Neue Cyr, Helvetica Neue, Helvetica, Arial, sans-serif`;const lines=wrapText(node.body,Math.max(140,faceWidth*0.54),bodyFont).slice(0,mobile?3:4);ctx.font=bodyFont;lines.forEach((line,index)=>ctx.fillText(line,0,34*textScale+index*bodySize*1.45));ctx.restore();}
...
function cvRotateBoxPoint(point,yaw,pitch){const scaled={x:point.x*1.32,y:point.y,z:point.z*1.18};const cy=Math.cos(yaw),sy=Math.sin(yaw),cp=Math.cos(pitch),sp=Math.sin(pitch);const x=scaled.x*cy+scaled.z*sy;const z=-scaled.x*sy+scaled.z*cy;const y=scaled.y*cp-z*sp;return{x,y,z:scaled.y*sp+z*cp};}
function drawCvBoxFace(face,centerX,centerY,focal,cameraZ){const points=face.points.map((point)=>cvProjectBoxPoint(point,centerX,centerY,focal,cameraZ));const avgZ=points.reduce((sum,point)=>sum+point.z,0)/points.length;const facing=Math.max(0.2,Math.min(1,(avgZ+face.depthBias+360)/820));const active=face.active;ctx.save();ctx.beginPath();points.forEach((point,index)=>{if(index)ctx.lineTo(point.x,point.y);else ctx.moveTo(point.x,point.y);});ctx.closePath();ctx.globalAlpha=(active?0.96:0.82)*facing;ctx.fillStyle=active?"rgba(0,0,0,0.98)":"rgba(0,0,0,0.93)";ctx.fill();ctx.strokeStyle=active?"rgba(255,255,255,0.25)":"rgba(255,255,255,0.12)";ctx.lineWidth=active?1.05:0.65;ctx.stroke();const center=points.reduce((sum,point)=>({x:sum.x+point.x/4,y:sum.y+point.y/4}),{x:0,y:0});const topWidth=Math.hypot(points[1].x-points[0].x,points[1].y-points[0].y);const bottomWidth=Math.hypot(points[2].x-points[3].x,points[2].y-points[3].y);const leftHeight=Math.hypot(points[3].x-points[0].x,points[3].y-points[0].y);const rightHeight=Math.hypot(points[2].x-points[1].x,points[2].y-points[1].y);const faceWidth=(topWidth+bottomWidth)/2;const faceHeight=(leftHeight+rightHeight)/2;const angle=Math.atan2(points[1].y-points[0].y,points[1].x-points[0].x);const localWidth=920,localHeight=570;const localScale=Math.max(0.12,Math.min(faceWidth/localWidth,faceHeight/localHeight));ctx.save();ctx.beginPath();ctx.moveTo(points[0].x,points[0].y);ctx.lineTo(points[1].x,points[1].y);ctx.lineTo(points[2].x,points[2].y);ctx.lineTo(points[3].x,points[3].y);ctx.closePath();ctx.clip();ctx.translate(center.x,center.y);ctx.rotate(angle);ctx.scale(localScale,localScale);ctx.globalAlpha=(active?0.96:0.46)*facing;ctx.fillStyle="#fff";ctx.textAlign="center";ctx.textBaseline="middle";ctx.font="700 18px Helvetica Neue Cyr, Helvetica Neue, Helvetica, Arial, sans-serif";ctx.fillText(face.label||cvBoxLabel(face.node),0,-128);ctx.font=`700 ${face.node.type==="hero"?74:54}px Helvetica Neue Cyr, Helvetica Neue, Helvetica, Arial, sans-serif`;ctx.fillText(face.node.title,0,-42);ctx.globalAlpha=(active?0.74:0.3)*facing;const bodyFont="21px Helvetica Neue Cyr, Helvetica Neue, Helvetica, Arial, sans-serif";ctx.font=bodyFont;const lines=wrapText(face.node.body,680,bodyFont).slice(0,width<700?3:4);lines.forEach((line,index)=>ctx.fillText(line,0,52+index*34));ctx.restore();ctx.restore();}
...
function drawCvBoxFace(face,centerX,centerY,focal,cameraZ){const points=face.points.map((point)=>cvProjectBoxPoint(point,centerX,centerY,focal,cameraZ));const avgZ=points.reduce((sum,point)=>sum+point.z,0)/points.length;const facing=Math.max(0.16,Math.min(1,(avgZ+face.depthBias+360)/820));const active=face.active;ctx.save();ctx.beginPath();points.forEach((point,index)=>{if(index)ctx.lineTo(point.x,point.y);else ctx.moveTo(point.x,point.y);});ctx.closePath();ctx.globalAlpha=(active?0.94:0.78)*facing;ctx.fillStyle=active?"rgba(0,0,0,0.96)":"rgba(0,0,0,0.9)";ctx.fill();ctx.strokeStyle=active?"rgba(255,255,255,0.28)":"rgba(255,255,255,0.13)";ctx.lineWidth=active?1.1:0.7;ctx.stroke();const p0=points[0],p1=points[1],p3=points[3];const ux={x:p1.x-p0.x,y:p1.y-p0.y};const vy={x:p3.x-p0.x,y:p3.y-p0.y};ctx.save();ctx.beginPath();ctx.moveTo(p0.x,p0.y);ctx.lineTo(p1.x,p1.y);ctx.lineTo(points[2].x,points[2].y);ctx.lineTo(p3.x,p3.y);ctx.closePath();ctx.clip();ctx.translate(p0.x,p0.y);ctx.transform(ux.x/1000,ux.y/1000,vy.x/1000,vy.y/1000,0,0);ctx.globalAlpha=(active?0.96:0.45)*facing;ctx.fillStyle="#fff";ctx.textAlign="center";ctx.textBaseline="middle";ctx.font="700 19px Helvetica Neue Cyr, Helvetica Neue, Helvetica, Arial, sans-serif";ctx.fillText(face.label||cvBoxLabel(face.node),500,350);ctx.font=`700 ${face.node.type==="hero"?88:58}px Helvetica Neue Cyr, Helvetica Neue, Helvetica, Arial, sans-serif`;ctx.fillText(face.node.title,500,458);ctx.globalAlpha=(active?0.74:0.32)*facing;const bodyFont="23px Helvetica Neue Cyr, Helvetica Neue, Helvetica, Arial, sans-serif";ctx.font=bodyFont;const lines=wrapText(face.node.body,700,bodyFont).slice(0,width<700?3:4);lines.forEach((line,index)=>ctx.fillText(line,500,550+index*36));ctx.restore();ctx.restore();}
function drawCvBoxFace(face,centerX,centerY,focal,cameraZ){const points=face.points.map((point)=>cvProjectBoxPoint(point,centerX,centerY,focal,cameraZ));const avgZ=points.reduce((sum,point)=>sum+point.z,0)/points.length;const facing=Math.max(0.18,Math.min(1,(avgZ+face.depthBias+360)/820));const active=face.active;ctx.save();ctx.beginPath();points.forEach((point,index)=>{if(index)ctx.lineTo(point.x,point.y);else ctx.moveTo(point.x,point.y);});ctx.closePath();ctx.globalAlpha=(active?0.96:0.82)*facing;ctx.fillStyle=active?"rgba(0,0,0,0.98)":"rgba(0,0,0,0.93)";ctx.fill();ctx.strokeStyle=active?"rgba(255,255,255,0.25)":"rgba(255,255,255,0.12)";ctx.lineWidth=active?1.05:0.65;ctx.stroke();const center=points.reduce((sum,point)=>({x:sum.x+point.x/4,y:sum.y+point.y/4}),{x:0,y:0});const topWidth=Math.hypot(points[1].x-points[0].x,points[1].y-points[0].y);const bottomWidth=Math.hypot(points[2].x-points[3].x,points[2].y-points[3].y);const leftHeight=Math.hypot(points[3].x-points[0].x,points[3].y-points[0].y);const rightHeight=Math.hypot(points[2].x-points[1].x,points[2].y-points[1].y);const faceWidth=(topWidth+bottomWidth)/2;const faceHeight=(leftHeight+rightHeight)/2;let angle=Math.atan2(points[1].y-points[0].y,points[1].x-points[0].x);if(angle>Math.PI/2)angle-=Math.PI;if(angle<-Math.PI/2)angle+=Math.PI;angle=Math.max(-0.22,Math.min(0.22,angle));const localWidth=920,localHeight=570;const localScale=Math.max(0.12,Math.min(faceWidth/localWidth,faceHeight/localHeight));ctx.save();ctx.beginPath();ctx.moveTo(points[0].x,points[0].y);ctx.lineTo(points[1].x,points[1].y);ctx.lineTo(points[2].x,points[2].y);ctx.lineTo(points[3].x,points[3].y);ctx.closePath();ctx.clip();ctx.translate(center.x,center.y);ctx.rotate(angle);ctx.scale(localScale,localScale);ctx.globalAlpha=(active?0.96:0.46)*facing;ctx.fillStyle="#fff";ctx.textAlign="center";ctx.textBaseline="middle";ctx.font="700 18px Helvetica Neue Cyr, Helvetica Neue, Helvetica, Arial, sans-serif";ctx.fillText(face.label||cvBoxLabel(face.node),0,-128);ctx.font=`700 ${face.node.type==="hero"?74:54}px Helvetica Neue Cyr, Helvetica Neue, Helvetica, Arial, sans-serif`;ctx.fillText(face.node.title,0,-42);ctx.globalAlpha=(active?0.74:0.3)*facing;const bodyFont="21px Helvetica Neue Cyr, Helvetica Neue, Helvetica, Arial, sans-serif";ctx.font=bodyFont;const lines=wrapText(face.node.body,680,bodyFont).slice(0,width<700?3:4);lines.forEach((line,index)=>ctx.fillText(line,0,52+index*34));ctx.restore();ctx.restore();}
function drawCvCubeWarpedLine(text,y,fontSize,warp){ctx.font=`700 ${fontSize}px Helvetica Neue Cyr, Helvetica Neue, Helvetica, Arial, sans-serif`;const glyphs=Array.from(text);const widths=glyphs.map((glyph)=>ctx.measureText(glyph).width);const total=widths.reduce((sum,value)=>sum+value,0);let cursor=-total/2;glyphs.forEach((glyph,index)=>{const glyphWidth=widths[index];const center=cursor+glyphWidth/2;const normal=total?center/(total/2):0;const edge=Math.min(1,Math.abs(normal));ctx.save();ctx.translate(center,y+normal*normal*fontSize*warp*0.28);ctx.rotate(normal*warp*0.18);ctx.scale(1+edge*edge*warp*1.35,1-edge*edge*warp*0.26);ctx.fillText(glyph,0,0);ctx.restore();cursor+=glyphWidth;});}
function drawCvBoxFace(face,centerX,centerY,focal,cameraZ){const points=face.points.map((point)=>cvProjectBoxPoint(point,centerX,centerY,focal,cameraZ));const avgZ=points.reduce((sum,point)=>sum+point.z,0)/points.length;const facing=Math.max(0.18,Math.min(1,(avgZ+face.depthBias+360)/820));const active=face.active;ctx.save();ctx.beginPath();points.forEach((point,index)=>{if(index)ctx.lineTo(point.x,point.y);else ctx.moveTo(point.x,point.y);});ctx.closePath();ctx.globalAlpha=(active?0.96:0.82)*facing;ctx.fillStyle=active?"rgba(0,0,0,0.98)":"rgba(0,0,0,0.93)";ctx.fill();ctx.strokeStyle=active?"rgba(255,255,255,0.25)":"rgba(255,255,255,0.12)";ctx.lineWidth=active?1.05:0.65;ctx.stroke();const center=points.reduce((sum,point)=>({x:sum.x+point.x/4,y:sum.y+point.y/4}),{x:0,y:0});const topWidth=Math.hypot(points[1].x-points[0].x,points[1].y-points[0].y);const bottomWidth=Math.hypot(points[2].x-points[3].x,points[2].y-points[3].y);const leftHeight=Math.hypot(points[3].x-points[0].x,points[3].y-points[0].y);const rightHeight=Math.hypot(points[2].x-points[1].x,points[2].y-points[1].y);const faceWidth=(topWidth+bottomWidth)/2;const faceHeight=(leftHeight+rightHeight)/2;let angle=Math.atan2(points[1].y-points[0].y,points[1].x-points[0].x);if(angle>Math.PI/2)angle-=Math.PI;if(angle<-Math.PI/2)angle+=Math.PI;angle=Math.max(-0.22,Math.min(0.22,angle));const localWidth=920,localHeight=570;const localScale=Math.max(0.12,Math.min(faceWidth/localWidth,faceHeight/localHeight));ctx.save();ctx.beginPath();ctx.moveTo(points[0].x,points[0].y);ctx.lineTo(points[1].x,points[1].y);ctx.lineTo(points[2].x,points[2].y);ctx.lineTo(points[3].x,points[3].y);ctx.closePath();ctx.clip();ctx.translate(center.x,center.y);ctx.rotate(angle);ctx.scale(localScale,localScale);const warp=active?0.3:0.18;ctx.globalAlpha=(active?0.96:0.46)*facing;ctx.fillStyle="#fff";ctx.textAlign="center";ctx.textBaseline="middle";drawCvCubeWarpedLine(face.label||cvBoxLabel(face.node),-128,18,warp*0.55);drawCvCubeWarpedLine(face.node.title,-42,face.node.type==="hero"?74:54,warp);ctx.globalAlpha=(active?0.74:0.3)*facing;const bodyFont="21px Helvetica Neue Cyr, Helvetica Neue, Helvetica, Arial, sans-serif";ctx.font=bodyFont;const lines=wrapText(face.node.body,680,bodyFont).slice(0,width<700?3:4);lines.forEach((line,index)=>{ctx.save();ctx.font=bodyFont;drawCvCubeWarpedLine(line,52+index*34,21,warp*0.28);ctx.restore();});ctx.restore();ctx.restore();}
...
window.PortfolioAnalytics?.track("project_open",{projectId:activeProjectKey,projectTitle:item?.title||""});
...
const opening=projectFocusTarget<0.5;if(opening)updateProjectMediaSafeMargin();else closeExpandedVideo();projectFocusTarget=opening?1:0;projectZoomTarget=0;projectZoomProgress=0;projectViewUi.classList.toggle("is-media-open",opening);ribbonVelocity=0;ribbonAutoSpeed=0;ribbonAutoPhaseStartedAt=0;ribbonAutoPausedUntil=performance.now()+(opening?600000:5000);if(opening){
window.PortfolioAnalytics?.track("image_open",{projectId:activeProjectKey,projectTitle:projectTitle?.textContent||""});
const centeredIndex=Math.round(ribbonOffset);ribbonOffset=centeredIndex;ribbonTargetOffset=centeredIndex;syncCenteredProjectTitle();const projectItems=getProjectItems();if(projectItems.length){const index=((centeredIndex%projectItems.length)+projectItems.length)%projectItems.length;openExpandedVideo(projectItems[index]);}}}
...
const index=((Math.round(ribbonTargetOffset)%visibleItems.length)+visibleItems.length)%visibleItems.length;applyProjectContent(visibleItems[index],index);}
function closeProject(){closeExpandedVideo();document.documentElement.dataset.activeProjectItemCount="0";projectFocusTarget=0;projectFocusProgress=0;projectZoomTarget=0;projectZoomProgress=0;transitionTarget=0;viewMode="sphere";activeProjectKey="";window.clearTimeout(projectTopNavTimer);setProjectTopNavFrosted(false);spherePage.classList.remove("is-project");projectViewUi.hidden=true;projectViewUi.classList.remove("is-media-open");}
function openCv(){closeProject();window.PortfolioAnalytics?.track("cv_open");viewMode="cv";transitionTarget=0;transitionProgress=0;cvWheelAccumulator=0;cvWheelLockedUntil=0;cvBoxYaw=-0.38;cvBoxPitch=0.22;cvBoxVelocity={yaw:0,pitch:0};setCvFace("center",false);updateCvSceneUi();spherePage.classList.add("is-cv");cvView.hidden=false;cvView.scrollTop=0;}
...
cvWorks?.addEventListener("click",closeCv);
cvView.addEventListener("wheel",handleCvWheel,{passive:false});
document.getElementById("cvContactLinks")?.addEventListener("click",(event)=>{const link=event.target.closest("a");if(link)window.PortfolioAnalytics?.track("contact_click");});
cvSectionButtons.forEach((button)=>button.addEventListener("click",()=>setCvFace(button.dataset.cvFace)));
[cvSphereSizeRange,cvElementScaleRange,cvOverlayDimRange].forEach((input)=>input?.addEventListener("input",updateCvSceneUi));
...
window.addEventListener("resize",resize);document.addEventListener("visibilitychange",syncVisibleVideoPlayback);resize();updateUi();updateProjectUi();initializeWaterEffects();const initialCvDisplay=bootstrapData.cvDisplay||window.PORTFOLIO_BOOTSTRAP?.cvDisplay||{};applyCvDisplay(initialCvDisplay);applyCvContacts(initialCvDisplay);loadItems(assets);Promise.all([loadStoredAssetsAsync(),loadStoredCvNodesAsync()]).then(([nextAssets,nextCvNodes])=>{assets=nextAssets;cvNodes=nextCvNodes;loadItems(assets);});render();})();
