...
const idleDefaults={transparency:100,darkening:100,frost:0};
...
function loadIdleSettings(){try{return{...idleDefaults,...JSON.parse(localStorage.getItem("portfolioSphere.idlePanelEffects.v2")||"{}")};}catch{return{...idleDefaults};}}
function updateIdleUi(save=true){idleInputs.forEach((input)=>{const key=input.dataset.idleSetting;idleState[key]=clampSetting(input.value,idleDefaults[key],0,100);const value=document.querySelector(`[data-idle-value="${key}"]`);if(value)value.textContent=String(Math.round(idleState[key]));});const alpha=1-idleState.transparency/100;const rgb=Math.round(28*(1-idleState.darkening/100));spherePage.style.setProperty("--idle-panel-alpha",alpha.toFixed(3));spherePage.style.setProperty("--idle-panel-rgb",String(rgb));spherePage.style.setProperty("--idle-panel-blur",`${(idleState.frost*0.18).toFixed(1)}px`);if(save){try{localStorage.setItem("portfolioSphere.idlePanelEffects.v2",JSON.stringify(idleState));}catch{}}}
function setUiIdle(idle){if(viewMode!=="sphere")return;spherePage.classList.toggle("is-ui-idle",idle);}
function scheduleUiIdle(){window.clearTimeout(uiIdleTimer);uiIdleTimer=window.setTimeout(()=>setUiIdle(true),120);}
function initializeIdleEffects(){idleState=loadIdleSettings();idleInputs.forEach((input)=>{const key=input.dataset.idleSetting;input.value=String(clampSetting(idleState[key],idleDefaults[key],0,100));input.addEventListener("input",()=>updateIdleUi(true));});updateIdleUi(false);const surfaces=[topbar,footerNav,document.getElementById("glassControls"),idleControls].filter(Boolean);const wakeUi=()=>{window.clearTimeout(uiIdleTimer);setUiIdle(false);};surfaces.forEach((surface)=>{surface.addEventListener("pointerenter",wakeUi);surface.addEventListener("pointerleave",scheduleUiIdle);});spherePage.addEventListener("pointermove",(event)=>{const isOverSurface=surfaces.some((surface)=>{const rect=surface.getBoundingClientRect();return event.clientX>=rect.left&&event.clientX<=rect.right&&event.clientY>=rect.top&&event.clientY<=rect.bottom;});if(isOverSurface)wakeUi();else scheduleUiIdle();},true);scheduleUiIdle();}
